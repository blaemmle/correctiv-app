...
- **`options.title` does not reach the browser tab.** expo-router 57 hands its
  ...
  (`build/ExpoRoot.js`), so react-navigation's own title bridge is off; `expo-router/head`
  is the path left open and needs a `HelmetProvider` this app does not mount; and the
  static export bakes in nothing either, because the root shell renders `null` until the
  fonts and store are ready, so `expo export` renders no screen at all and every page
  ships `<title data-rh="true"></title>`. Setting the option looks right in code and
  changes nothing anybody can see. → `ScreenHeader.web.tsx` sets `document.title` itself
  and restores the previous one on unmount ([ADR 0030](adr/0030-the-platforms-header-and-ours-on-web.md)).
