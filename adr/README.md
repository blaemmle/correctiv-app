...
Sixty-five records shaped this repo. Read them when you want to know *why* something
...
| [0004](0004-react-native-pivot.md) | Move to React Native / Expo, with a web target | accepted; a sentence and a half on the web reader's sandbox retired by 0065, the reason behind them left standing |
...
| [0017](0017-native-rendering-as-the-rule-a-webview-for-the-exception.md) | Native rendering as the rule, a webview for the exception | accepted; corrects a frame-header claim in `ReaderView.web.tsx`, one open question named; its claim that the sanitiser discards a frame retired by 0065 |
...
| [0061](0061-a-submission-is-an-issue-and-ci-makes-the-pull-request.md) | A submission is an issue, and CI makes the pull request | accepted, **built the same day**, its `[texte]` kind built by 0062; seven decisions, carrying out 0058 §1 and revising its §2 after the product owner found the clipboard and GitHub's editor unreasonable for the newsroom: one click opens a prefilled GitHub issue and a workflow ported from Learn6502 turns it into a pull request authored by the person, with a German summary derived from the diff of the two parsed documents; a submission's kind is its title prefix, `[startseite]` built and `[texte]` named for the German catalogue with ADR 0056's picker; it runs by itself only for the organisation and its collaborators, and a maintainer's manual run is bound to the text they read; CI on the pull request needs a token held by CI, and without it the workflow checks the file itself and says so; the address is bounded at 4000 characters after measuring GitHub's sign-in redirect, with the clipboard past it; nothing from the issue reaches a shell as code or the pull request's body as an instruction, held by tests that keep the security review's inputs; strikes the mechanism of 0058 §2, one sentence of its alternatives and its third open item, five open items named |
| [0062](0062-the-texts-submission-may-change-wordings-and-nothing-else.md) | The texts submission may change wordings, and nothing else | accepted, **built the same day**, not run against github.com; nine decisions, carrying out 0061 §3's `[texte]` kind with the dev server's own validator, write and German, replacing the one-file assertion with an allow-list of catalogue files and a proof that reads each changed file with the scanner against `main` and lets only the submitted literals differ, refusing a wording over a thousand characters, with a character a reviewer cannot see or with a placeholder of another kind, refusing a submission whole for one bad entry or an id written twice, reading an issue only in the shape the workbench writes, and sending the wordings one per line after measuring that 27 realistic edits still fit the address; after a cold review of #263, a maintainer approves the text the automation quoted and the run takes that text, running by itself needs write access, and splitting the token's job is deferred with its reason; strikes two clauses and one open item of 0061 and one open item of 0056, four open items named |
| [0063](0063-android-back-is-the-navigators-and-the-onboarding-answers-its-own.md) | Android back is the navigator's, and the onboarding answers its own | accepted, **built the same day**; four decisions after walking every case of #120 on Android 16 with the button and the gesture and the web export in a browser: the navigator answers back and only the onboarding, whose pages are not routes, answers its own, stepping back a page and backing out of the first page as its skip, which a first launch had turned into leaving the app; predictive back stays off because React Native keeps a back callback enabled and the preview was measured not to appear; back on the door leaves the app on purpose; the web answers nothing. Answers the native-arrow item of 0026 §9 and 0030, strikes nothing, iOS unwalked |
| [0064](0064-ci-cd-for-a-public-repository.md) | CI/CD for a public repository | accepted; Actions/Fastlane, secretless PR builds, public review APK and separate TestFlight app from `main`, release-triggered store candidates; implementation pending |
| [0065](0065-embeds-inline-from-a-short-list-and-a-link-for-the-rest.md) | Embeds render inline from a short list, and the rest become a link | accepted, **built the same day**; seven decisions after measuring what each embed in 300 posts loads: Datawrapper, CORRECTIV's own apps, 23degrees, Flourish and DocumentCloud render, the last as the borderline case, and the list is a proposal the newsroom may change in one place; YouTube, LinkedIn and Instagram become a link to the browser, a correctiv.org post a link into the reader; the document forbids itself script by policy, which lets the web frame carry `allow-scripts` for the embeds; §4, §5 and §7 revised after a cold review found a `<meta>` refresh turning the web reader into a page on the app's own origin, so the document builder holds every body to an allowlist over a parsed tree, which a same-day re-check forced after two bodies went through a regex denylist, and Datawrapper and Flourish frames get no origin of their own; strikes a sentence and a half in 0004 and one in 0017, iOS and Android unrun |
