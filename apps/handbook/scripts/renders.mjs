/**
 * Opens the handbook in a browser and fails if it did not render.
 *
 ...
 * `npm run build:handbook` was green, CI was green, and `npm run handbook` — the one
 ...
 *    something mounted.
 * 2. Nothing on the page carries `data-view-failed` — what mounted is the handbook
 ...
 *    an effect reads as fine. Measured on both modes: zero on a healthy handbook, in
 ...
 * passes, which is what `/workbench` and a pair of eyes are for. It opens one
 ...
 * measured on. CI runs both in the job that already builds the handbook.
 */
 ...
 * handbook — about a server that was merely slower than the browser, which is a
 ...
 * direction, and the remedy is `npm run build:handbook` and five seconds. Without
...
function refuseStaleDist(dist) {
  ...
  for (const file of files) {
    if (file === '' || file.startsWith('apps/handbook/dist/')) continue;
    let at;
    try {
  ...
  if (newest.at > built) {
    ...
    throw new Reported(
      `apps/handbook/dist is ${age}s older than ${newest.file}, so this would have judged ` +
        `a site nobody is looking at.\nRun \`npm run build:handbook\` and try again.`,
    );
  }
}
  ...
  if (verdict.failed !== null) {
    throw new Reported(
      `${mode}: the shell mounted and the view inside it did not. ${where} serves the error ` +
        ...
        `page full of words and none of them the handbook's.\n${said}`,
    );
  }
  if (faults.length > 0) {
    throw new Reported(
      `${mode}: the handbook mounted, and the browser logged an error while it did. ` +
        `${where} serves a page that is broken in a way this cannot see the extent of.\n${said}`,
    );
  }

  console.log(`${mode}: the handbook renders — “${verdict.title}”, ${verdict.words.slice(0, 60)}…`);
}
