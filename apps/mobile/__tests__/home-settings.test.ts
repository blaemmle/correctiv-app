...
import { OUT, render } from '../scripts/generate-home-settings.mjs';
...
const DECLARATIONS = resolve(__dirname, '../src/lib/home/settings.ts');

/** This package's manifest, which is where the generator's command is spelled twice. */
const APP_PACKAGE = resolve(__dirname, '../package.json');
const REPO_PACKAGE = resolve(__dirname, '../../../package.json');
...
describe('the settings a module declares beside itself', () => {
  ...
  it('tells a reader of the artefact to run a command that exists', () => {
    ...
    expect(existsSync(resolve(__dirname, '../../..', named!))).toBe(true);
