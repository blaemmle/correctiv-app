 ...
 *
 * A union rather than one flat object, because `backLabel` and `children` are
...
type ScreenTitle = {
   ...
   *
   * Drawn by the platform's header on iOS and Android, and on web it is the
   * browser tab. Required, because until ADR 0030 no `Stack.Screen` set one and
   * every pushed route in the published export carried an empty `<title>` — a
   * defect nothing could see, since a missing title looks like the address bar
   * doing its job.
   */
  title: string;
};
